...
class AuxiliaryCriterion(LabelSmoothedCrossEntropyCriterion):
    ...
    def forward(self, model, sample, reduce=True, show=False):
        ...
        net_output, net_output_regularization = model(**sample["net_input"])
        ...
        regularization_loss, _ = self.compute_loss(model, net_output_regularization, sample, reduce=reduce)
        p_norm = torch.norm(1 - net_output[-1]["mask"][0], p=2) / src_len
        regularization_loss_final = -regularization_loss + self.mask_loss_weight * p_norm
        ...
        del regularization_loss, p_norm

        return regularization_loss_final, sample_size, logging_output
